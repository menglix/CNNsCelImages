...
# it corresponds to VGG-type CNN of our straightened Res50 with identity shortcuts 
...
# code adapted from: https://ctmakro.github.io/site/on_learning/resnet_keras.html
...
filepath="/space/data/deepyeastmodel/Res50Plain/Res50PlainMaxweights.{epoch:02d}-{val_acc:.2f}.hdf5"
...
def conv_block(nip,nop,layers,std):
    def unit(x):
        for i in range(layers):
            if i==0:
                x = Conv2D(nop, (1,1), strides=(std,std),padding='same',kernel_regularizer=regularizers.l2(0.0005),kernel_initializer='glorot_normal')(x)
                x = BatchNormalization(axis=-1)(x)
                x = relu(x)
                x = Conv2D(nop, (3,3), padding='same',kernel_regularizer=regularizers.l2(0.0005),kernel_initializer='glorot_normal')(x)
                x = BatchNormalization(axis=-1)(x)
                ...
                x = Conv2D(nop, (1,1), strides=(1,1),padding='same',kernel_regularizer=regularizers.l2(0.0005),kernel_initializer='glorot_normal')(x)
                x = BatchNormalization(axis=-1)(x)
                x = relu(x)
                x = Conv2D(nop, (3,3), padding='same',kernel_regularizer=regularizers.l2(0.0005),kernel_initializer='glorot_normal')(x)
                x = BatchNormalization(axis=-1)(x)
...
# conv2_x
i = conv_block(64,64,3,1)(i) # 16*16


# conv3_x
i = conv_block(64,64,4,1)(i) 
i = MaxPooling2D(pool_size=(2, 2),padding='same')(i) # 8*8

# conv4_x
i = conv_block(64,64,6,1)(i) 
i = MaxPooling2D(pool_size=(2, 2),padding='same')(i) # 4*4

# conv5_x
i = conv_block(64,64,3,1)(i) 
...
with open(output_dir+'/dyresnet50PlainMax'+'.pkl', 'wb') as file_pi:
...
plt.savefig('/home/menglix/result/deepyeast/dyRes50PlainMaxaccuracy.pdf')
plt.clf()
